...
//   const [selectedReport, setSelectedReport] = useState(""); // Store only one report type
//   const [selectedReportType, setSelectedReportType] = useState(""); // Use useState to declare this
//   const [files, setFiles] = useState({}); // Store files for each report
...
//   // Handle file selection
//   const handleFileChange = (e, report) => {
//     const selectedFile = e.target.files[0];
//     setFiles((prevFiles) => ({ ...prevFiles, [report]: selectedFile })); // Update file for the specific report
//   };
...
//       // Append each report file
//       Object.keys(files).forEach((reportName) => {
//         formData.append("reportFile", files[reportName]); // Send files as an array
//         formData.append("reportName", reportName); // Send report names as an array
//       });
...
//                 type="checkbox"
//                 id={report}
//                 checked={selectedReport.includes(report)}
//                 onChange={() => handleReportToggle(report)}
//                 className="form-checkbox h-5 w-5 text-teal-600 rounded focus:ring-2 focus:ring-teal-500"
//               />
//               <label htmlFor={report} className="ml-3 flex-grow cursor-pointer">
//                 {report}
//               </label>
//               {selectedReport.includes(report) && (
//                 <div>
//                   <label
//                     htmlFor={`file-upload-${report}`}
//                     className="text-sm text-white bg-teal-600 p-2 rounded file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100"
//                   >
//                     {files[report] ? "Change File" : `Upload file`}
//                   </label>
//                   <input
//                     id={`file-upload-${report}`}
//                     type="file"
//                     onChange={(e) => handleFileChange(e, report)} // Pass report name to handleFileChange
//                     className="hidden" // Hide the input itself
//                     aria-label={`Upload file for ${report}`}
//                   />
//                   {files[report] && (
//                     <p className="text-sm text-gray-600 mt-2">
//                       Selected file: {files[report].name}
//                     </p>
//                   )}
//                 </div>
//               )}
...
//   const [selectedPatient, setSelectedPatient] = useState(null);
//   const [selectedReport, setSelectedReport] = useState(""); // Store only one report type
//   const [selectedReportType, setSelectedReportType] = useState("Lab Report"); // Use useState to declare this
//   const [files, setFiles] = useState({}); // Store files for each report
//   const [multipleReports, setMultipleReports] = useState([]);
//   const [showSuccess, setShowSuccess] = useState(false);
//   const [loading, setLoading] = useState(false);
...
//       setSelectedReportType("Diagnostic Report");
//     } else {
//       setSelectedReportType("Lab Report"); // Handle case where the report is not found
//     }
//   }, [selectedReport]);
...
//     // Check for selected patient and report selection
//     if (!selectedPatient || !selectedReport.length) {
//       alert(
//         "Please select a patient and at least one report type before uploading."
//       );
//       return;
//     }
...
//       // Append each selected report's file to formData without including the report name
//       selectedReport.forEach((report) => {
//         if (files[report]) {
//           formData.append("reports", files[report]); // Add each file under the "reports" field
//         }
//       });

//       // Make axios POST request to upload files along with metadata
//       const response = await axios.post(
